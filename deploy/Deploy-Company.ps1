...
# Cloudinary keys master .env se carry karo (photos/documents storage -
# har company app ko bhi chahiye, warna uploads base64 fallback me jayenge)
$cloudName = ''
$cloudPreset = ''
if (Test-Path $backupPath) {
  $masterEnv = Get-Content $backupPath -Raw
  if ($masterEnv -match 'VITE_CLOUDINARY_CLOUD_NAME=(\S+)') { $cloudName = $Matches[1] }
  if ($masterEnv -match 'VITE_CLOUDINARY_UPLOAD_PRESET=(\S+)') { $cloudPreset = $Matches[1] }
}
...
VITE_CLOUDINARY_UPLOAD_PRESET=$cloudPreset
"@
