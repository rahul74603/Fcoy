...
if (-not $company)                        { throw "❌ companies.json me '$Code' nahi mila. Available: $($registry.PSObject.Properties.Name -join ', ')" }
if ($company.alreadyDeployed -eq $true)   { throw "❌ '$Code' = MASTER APP — ise is script se deploy mat karo (build + firebase deploy manually hota hai)." }

$required = 'projectId','apiKey','authDomain','storageBucket','messagingSenderId','appId'
...
if (Test-Path $envPath) {
  ...
  Write-Host "✓ Master .env ka backup: $backupPath" -ForegroundColor DarkGray
}
...
Write-Host "✓ .env ($Code ke Firebase keys se) likh diya" -ForegroundColor Green
...
try {
  ...
  if (Test-Path $backupPath) {
    ...
    Write-Host "✓ .env wapas master pe restore ho gaya" -ForegroundColor DarkGray
  }
}
