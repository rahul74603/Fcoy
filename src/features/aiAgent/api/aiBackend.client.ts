...
export async function callGroq(payload: {
  ...
  toolChoice?: unknown;
  model?: string;
}): Promise<{ content: string; message: { role: string; content: string; tool_calls?: any[] }; finishReason: string | null; model: string }> {
  const fn = httpsCallable<typeof payload, any>(functions()!, 'aiGroq');
  const res = await fn(payload);
  // Server returns { message, finishReason, model, content } (content kept for
  // back-compat). Normalize so callers can run the tool loop.
  const message = res.data?.message ?? { role: 'assistant', content: res.data?.content ?? '' };
  return {
    content: res.data?.content ?? message.content ?? '',
    message,
