...
async function groqViaBackend(opts: GroqOpts): Promise<any> {
  // Callable expects { messages, temperature, maxTokens, responseFormat }.
  const payload: any = {
    messages: opts.messages.map((m) => ({ role: m.role, content: m.content })),
    temperature: opts.temperature ?? 0.1,
    maxTokens: opts.maxTokens ?? 1200,
  };
  // Tool calling via backend: pass tools; the function forwards them.
  if (opts.tools) payload.tools = opts.tools;
  try {
    const content = await backendCallGroq(payload);
    // Backend returns the assistant text; for tool calls we parse the
    // standard Groq JSON shape when present (server includes raw choice).
    return parseBackendGroq(content);
  } catch (e: any) {
    throw mapBackendError(e);
  }
}

/** Backend aiGroq returns content string; if it embedded raw JSON use it. */
function parseBackendGroq(content: string): any {
  if (typeof content !== 'string') return content;
  try {
    const parsed = JSON.parse(content);
    if (parsed?.choices?.[0]?.message) return parsed;
  } catch { /* plain text */ }
