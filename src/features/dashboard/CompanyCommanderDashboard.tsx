      ...
      {loading ? (
        ...
        <>
          ...
          <div className="overflow-hidden rounded-2xl border-2 border-red-200 bg-white shadow-md">
            <div className={`flex flex-wrap items-center justify-between gap-3 border-b border-red-100 px-5 py-4 ${awayOpen ? 'bg-gradient-to-r from-red-50 to-amber-50' : 'bg-red-100/80'}`}>
              <button type="button" onClick={() => setAwayOpen(o => !o)} aria-expanded={awayOpen} className="group flex flex-1 items-center gap-3 text-left">
                <div className={`flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full transition-all duration-300 ${awayOpen ? 'bg-white text-red-500 rotate-180' : 'bg-red-600 text-white'}`}>
                  <ChevronDown size={15} />
                </div>
                <div><p className="text-[10px] font-black uppercase tracking-widest text-red-700">Live attention roster</p><h2 className="mt-1 text-sm font-black uppercase text-slate-800">Trainees Not On Field ({awayTrainees.length}) — Full Details</h2><p className="mt-1 text-[10px] text-slate-500">Who is away, why, and expected return{awayOpen && awayTrainees.length > AWAY_VISIBLE_ROWS ? ` · pehle ${AWAY_VISIBLE_ROWS} dikh rahe hain, baaki scroll karo` : ''}</p></div>
                <span className={`hidden sm:flex items-center rounded-lg px-2.5 py-1.5 text-[9px] font-black uppercase tracking-wide ${awayOpen ? 'bg-white/70 text-slate-600' : 'bg-red-600 text-white shadow-sm'}`}>
                  {awayOpen ? 'Hide' : 'Click here for more'}
                </span>
              </button>
          ...
          {/* ═══ CLEAN ROSTER BOARD ═══ */}
          <div className={`rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm ${platoonOpen ? '' : 'bg-military-50/70'}`}>
            <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
              <button type="button" onClick={() => setPlatoonOpen(o => !o)} aria-expanded={platoonOpen} className="group flex flex-1 items-center gap-3 text-left">
                <div className={`flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full transition-all duration-300 ${platoonOpen ? 'bg-slate-100 text-slate-500 rotate-180' : 'bg-military-700 text-white'}`}>
                  <ChevronDown size={15} />
                </div>
                <div><p className="text-[10px] font-black uppercase tracking-widest text-military-600">Roster command board</p><h2 className="mt-1 text-sm font-black uppercase text-slate-800">Platoon-wise live strength</h2></div>
                <span className={`hidden sm:flex items-center rounded-lg px-2.5 py-1.5 text-[9px] font-black uppercase tracking-wide ${platoonOpen ? 'bg-slate-100 text-slate-600' : 'bg-military-700 text-white shadow-sm'}`}>
                  {platoonOpen ? 'Hide' : 'Click here for more'}
                </span>
              </button>
              <button onClick={() => focusRoster('ALL')} className="rounded-lg bg-military-800 px-3 py-2 text-[10px] font-black uppercase text-white hover:bg-military-700">Open full roster →</button>
            </div>
            {platoonOpen && <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">{platoons.filter(p => p !== 'ALL').map(p => { const members = trainees.filter(t => t.platoon === p); const presentCount = members.filter(t => getDashboardAttnCode(t) === 'P').length; const pct = members.length ? Math.round(presentCount / members.length * 100) : 0; return <button key={p} onClick={() => focusRoster(p)} className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-left transition hover:-translate-y-0.5 hover:border-military-400 hover:bg-white hover:shadow-md"><div className="flex items-center justify-between"><span className="text-xs font-black text-slate-800">{p}</span><span className={`rounded-full px-2 py-1 text-[9px] font-black ${pct >= 90 ? 'bg-green-100 text-green-700' : pct >= 75 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>{pct}%</span></div><div className="mt-3 h-2 rounded-full bg-slate-200"><div className={`h-full rounded-full ${pct >= 90 ? 'bg-green-500' : pct >= 75 ? 'bg-amber-500' : 'bg-red-500'}`} style={{ width: `${pct}%` }} /></div><div className="mt-3 flex justify-between text-[10px] font-bold"><span className="text-slate-600">{members.length} total</span><span className="text-green-700">{presentCount} present</span><span className="text-red-600">{members.length - presentCount} away</span></div></button>; })}</div>}
          </div>
