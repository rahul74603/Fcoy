...
export const ModuleReportButton: React.FC<Props> = ({ module, stats = [], rows = [] }) => {
  ...
  const coyShort = (unitConfig.companyShort || unitConfig.companyName || 'COMPANY').toUpperCase();
  const unitLabel = `${coyShort} · ${unitConfig.parentUnit || 'TRAINING COMMAND'}`;
  const now = new Date();
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 p-4"><div className="max-h-[92vh] w-full max-w-5xl overflow-auto rounded-xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-slate-200 p-4"><div><h2 className="font-black text-slate-900">{config.title} · Preview</h2><p className="text-xs text-slate-500">Live itemized statement · Print karke PDF save karein</p></div><button onClick={() => setOpen(false)}><X size={18}/></button></div>
      <div id="inline-command-report" className="p-8"><div className="head"><div><div className="brand">{unitLabel}</div><div className="sub">BSF TRAINING CENTRE MANAGEMENT SYSTEM</div></div><div className="meta">OFFICIAL STATEMENT<br/>Date: {date}<br/>Time: {time}</div></div><div className="title">{config.emoji} {config.title}</div><div className="sub">{config.subtitle}</div><div className="rule"/><div className="notice"><b>Statement:</b> Active Batch · Live itemized records · Official use</div>
