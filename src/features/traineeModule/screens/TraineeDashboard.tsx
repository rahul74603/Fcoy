...
export const TraineeDashboard: React.FC = () => {
  ...
  const { activeBatch, currentBatch, loading: batchLoading } = useBatch();
  const batch = currentBatch || activeBatch;
  ...
  useEffect(() => {
    ...
    loadData();
  }, [user, batch, batchLoading, selectedDate]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const traineeSnap = await getDocs(query(collection(db, 'trainees'), where('batchId', '==', batch.id)));
      const tList: any[] = [];
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="max-w-md w-full bg-white border border-slate-200 rounded-xl p-6 text-center">
          <h1 className="text-lg font-black text-slate-800 uppercase">Koi running batch nahi mila</h1>
          <p className="text-sm text-slate-600 mt-2">
            Trainee dashboard tab khulega jab ek active batch ho. Staff ke liye Company Commander dashboard use karo.
          </p>
          <a href="/commander" className="mt-4 inline-block bg-green-800 text-white text-xs font-bold uppercase tracking-wider px-4 py-2 rounded">
            Commander dashboard
          </a>
          <button type="button" onClick={logout} className="mt-3 block mx-auto text-xs font-bold text-slate-500 underline">
            Logout
          </button>
        </div>
      </div>
